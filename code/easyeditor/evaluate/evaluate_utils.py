...
import boto3
import json
# from ..util_overall import system_msg_eval, system_msg_qa
...
# system_msg_eval = """You are an expert evaluator. Given two texts, determine if they are semantically related.

# system_msg_eval = "You are a helpful assistant."
# Rules:
# - Output '1' if the texts share the same main concept, topic, or activity
# - Output '1' if one text is a more detailed version of the other  
# - Output '1' if both texts refer to the same hobby, activity, or interest
# - Output '0' only if the texts are completely unrelated
# - Do not provide any explanation, just output '1' or '0'"""

content_template = """
Given a question, gold target, and predicted answer, output:
1 if the predicted answer is fully correct.
0 if it is incorrect.

Correct means:
- Contains all important information from the gold target.
- Contains no contradictions or irrelevant/repeated info.

Examples of 1 (correct):
Q: What are the names of Barack Obama's children?
Gold: Malia Obama and Sasha Obama
Pred: sasha and malia obama
Pred: Malia and Sasha Obama are the names of Barack Obama's children.

Examples of 0 (incorrect):
Q: What are the names of Barack Obama's children?
Gold: Malia and Sasha
Pred: Malia.
Pred: Malia, Sasha, and Susan.
Pred: Malia and Sasha, Malia and Sasha, Malia and Sasha, Malia and Sasha
...
Return only "1" or "0", with no text around it.
""".strip()
...
def claude_judge(ground_truth, prediction):
    """
    Judge using Claude via AWS Bedrock.
    Uses the same prompt format as the local-llm-judge function.
    """
    user_message = f"Text 1: {ground_truth} \nText 2: {prediction}"

    try:
        client = boto3.client(service_name="bedrock-runtime", region_name="us-east-1")
        # model_id = "us.anthropic.claude-haiku-4-5-20251001-v1:0" # quota: 250 requests / minute
        # model_id = "us.anthropic.claude-sonnet-4-5-20250929-v1:0" # quota: 200 requests / minute
        model_id = "us.anthropic.claude-3-7-sonnet-20250219-v1:0"  # quota: 250 requests / minute
        
        body = json.dumps({
            "anthropic_version": "bedrock-2023-05-31",
            "max_tokens": 2,
            "temperature": 0.0,
            "system": system_msg_eval,
            "messages": [
                {
                    "role": "user",
        ...
        claude_ans = response_body.get("content", [{}])[0].get("text", "").strip()
        
        llm_score = 1.0 if claude_ans == "1" else 0.0
...
def test_prediction_acc_LLM_judge_easyedit(model, tok, hparams, prompts, targets, device, locality=False):
...
def evaluate_response(hparams, model_eval, tok_eval, prompt_qa, output_qa, label, device_eval):
    ...
    if output_qa.lower() in label.lower() or label.lower() in output_qa.lower():  # Exact and partial match
        ...
        user_msg_eval = content_template.format(
            question=prompt_qa,
            target=label,
            predicted_answer=output_qa
        )
...
def test_prediction_acc_llm_judge(hparams, model_qa, tok_qa, model_eval, tok_eval, device_eval, prompt_qa, label, pre_or_post, vanilla_generation=False, system_msg=system_msg_qa):
    ...
    if isinstance(prompt_qa, list):
        for i, prompt in enumerate(prompt_qa):
            ...
            return test_prediction_acc_single_llm_judge(hparams, model_qa, tok_qa, model_eval, tok_eval, device_eval, prompt, label_, system_msg)
    else:
        return test_prediction_acc_single_llm_judge(hparams, model_qa, tok_qa, model_eval, tok_eval, device_eval, prompt_qa, label, system_msg)

def test_prediction_acc_single_llm_judge(hparams, model_qa, tok_qa, model_eval, tok_eval, device_eval, prompt_qa, label, system_msg_qa):
...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=True, system_msg=None, system_msg_eval=system_msg_eval, evaluation_type=None, additional_token_len=0, user_msg_eval_format=None):
    if vanilla_generation:
        ...
        for prompt, target_new in zip(prompts, targets):
            ...
            max_new_tokens_len = int(len(target_new_tokens) * 1) + additional_token_len # 

            if system_msg:
                if 'gpt-j' in hparams.model_name.lower():
                    # GPT-J doesn't support chat templates, handle it like the else case but with system message
                    combined_prompt = system_msg + ' ' + prompt
                    prompt_tok = tok(combined_prompt, return_tensors="pt").to(f"cuda:{device}")
                    ...
                    messages = [{"role": "user", "content": system_msg + ' ' + prompt}]
                    terminators = [tok.eos_token_id, tok.convert_tokens_to_ids("<|eot_id|>")]
                    ...
                    generated_tokens = output_ids[0][msg_tokenized['input_ids'].shape[-1]:].detach().cpu().numpy().tolist()
                    decoded_output = tok.decode(generated_tokens, skip_special_tokens=True).replace('\n', ' ').strip().rstrip('.')
                else:
            ...
            if locality:
                ...
                if target_new == decoded_output or target_new.lower() in decoded_output.lower():
                    results.append(1.0)
                    print(f"===== String-level match | label: {target_new} | response: {decoded_output} =====")
                elif evaluation_type == "claude-judge":
                    claude_score = claude_judge(target_new, decoded_output)
                    results.append(claude_score)
                    print(f"===== claude-judge | label: {target_new} | response: {decoded_output} | evaluation: {claude_score} =====")
                elif evaluation_type == "local-llm-judge":
                    judge_model = hparams.judge_model
                    judge_tok = hparams.judge_tok
                    if user_msg_eval_format:
                        user_msg_eval = user_msg_eval_format.format(target_new=target_new, decoded_output=decoded_output)
                    else:
                        user_msg_eval = f"""Text 1: {target_new} \nText 2: {decoded_output}"""
                    messages_eval = [{"role": "system", "content": system_msg_eval}, {"role": "user", "content": user_msg_eval}]
                    terminators = [judge_tok.eos_token_id, judge_tok.convert_tokens_to_ids("<|eot_id|>")]
                    msg_tokenized = judge_tok.apply_chat_template(messages_eval, add_generation_prompt=True, return_tensors='pt', return_dict=True).to(judge_model.device)
                    output_ids = judge_model.generate(**msg_tokenized, max_new_tokens=1, eos_token_id=terminators, do_sample=False, pad_token_id=judge_tok.eos_token_id)
                    judge_response = judge_tok.decode(output_ids[0][msg_tokenized['input_ids'].shape[-1]:], skip_special_tokens=True).replace('\n', ' ').strip().rstrip('.')
                    print(f"===== Local-LLM-judge | label: {target_new} | response: {decoded_output} | evaluation: {judge_response} =====")
                    if str(judge_response) == '1':
                        results.append(1.0)
                    else:
                        results.append(0.0)
                else:
