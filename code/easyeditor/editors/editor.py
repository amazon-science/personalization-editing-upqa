...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if type(self.model_name) is str:
            ...
            elif 'llama' in self.model_name.lower() or 'vicuna' in self.model_name.lower() or 'olmo' in self.model_name.lower() or 'phi' in self.model_name.lower():
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name, **model_kwargs)
    ...
    def edit_requests(self,
        ...
        def edit_func(request):
            ...
            if self.alg_name == 'IKE' or self.alg_name == 'ICE':
                ...
                icl_examples = None
            exec_time = round(time() - start, 2)
            return edited_model, weights_copy, icl_examples, exec_time

        def edit_evaluation(all_metrics, request, edited_model, idx, test_generation, icl_examples, exec_time, **kwargs):
            eval_metric= kwargs['eval_metric'] if 'eval_metric' in kwargs.keys() else 'exact match'
            if self.alg_name == 'IKE' or self.alg_name == 'ICE':
        ...
        if sequential_edit:
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                edited_model, weights_copy, icl_examples, exec_time = edit_func(request)
                edit_times.append(exec_time)
            ...
            for i, request in enumerate(requests):
                edit_evaluation(all_metrics, request, edited_model, i, test_generation, icl_examples, edit_times[i], **kwargs)
        else:
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                edited_model, weights_copy, icl_examples, exec_time = edit_func(request)
                edit_evaluation(all_metrics, request, edited_model, i, test_generation, icl_examples, exec_time, **kwargs)
                if self.alg_name == 'KN' or self.alg_name == 'GRACE' or self.alg_name == 'WISE':
                    with torch.no_grad():
                        weights_copy()
                elif self.alg_name == 'LoRA' or self.alg_name == 'QLoRA' or self.alg_name == 'DPO':
        ...
        for metric in all_metrics:
            ...
            if "product_recommendation_qa" in metric['requested_rewrite'].keys():
                metric['requested_rewrite']['product_recommendation_qa']['product_recommendation'] = metric['requested_rewrite']['product_recommendation_qa']['yes_question']['prompt']
    ...
    def generate_edit(
        ...
        **kwargs
        ...
        def post_edit_results(all_results, request, edited_model, idx, eval_metric, test_generation, icl_examples, exec_time, **kwargs):
            if self.alg_name == 'IKE':
        ...
        if sequential_edit:
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                ...
                edit_times.append(exec_time)
            for i, request in enumerate(requests):
                post_edit_results(all_results, request, edited_model, i, eval_metric, test_generation, icl_examples, edit_times[i], **kwargs)
        else:
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                edited_model, weights_copy, icl_examples, exec_time = edit_func(request)
                post_edit_results(all_results, request, edited_model, i, eval_metric, test_generation, icl_examples, exec_time, **kwargs)
                if self.alg_name == 'KN' or self.alg_name == 'GRACE' or self.alg_name == 'WISE':
                    with torch.no_grad():
                        weights_copy()
                elif self.alg_name == 'LoRA' or self.alg_name == 'QLoRA' or self.alg_name == 'DPO':
