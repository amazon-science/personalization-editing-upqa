...
from run_edit import load_data
...
system_msg_judge = """You are a lenient evaluator checking if an answer contains correct information. Output ONLY '1' if correct, or '0' if incorrect.

IMPORTANT: Be very lenient. Accept answers that:
- Contain the key fact (even with extra context)
- Use abbreviations
- Have different wording but same meaning
- Include the date/name even in a longer sentence
- Use synonyms or related terms

Only output 0 if the answer is factually wrong or missing the key information entirely."""
...
def exact_match_evaluation(pred, label):
    ...
    if isinstance(label, list):
        correct_answers = [ans.lower().strip().rstrip('.,;:!?') for ans in label]
    else:
        label = label.lower().strip()
        correct_answers = [ans.strip().rstrip('.,;:!?') for ans in label.split(',')]
    # return 1 if any(pred == ans for ans in correct_answers) else 0
    return 1 if any(pred in ans for ans in correct_answers) else 0


def gpt_evaluation(pred, label):
    """Use AWS Bedrock to evaluate semantic similarity between prediction and correct answer."""
    # Format label properly - if it's a list, convert to readable format
    if isinstance(label, list):
        label_formatted = ' OR '.join([f'"{ans}"' for ans in label])
    else:
        label_formatted = f'"{label}"'
    
    prompt = f"""Correct answer: {label_formatted}
Input answer: "{pred}"

Is the input answer correct? Output only 1 or 0."""
    response = get_bedrock_judge_response(prompt, judge_model_id, system_msg_judge, max_new_tokens=3)
    print(f'Claude eval | label: {label_formatted} | pred: "{pred}" | eval: "{response}"')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--results_dir', default='../results/general_capability1', type=str)
    ...
    # Load edit data
    edit_questions, edit_targets, edit_subjects, questions_paraphrased, implicit_qa, product_recommendation_qa, user_preference = load_data('../data/prefeval_pro/prefeval_pro_balanced.json', 100)
    
    hparams = editing_hparams.from_hparams(os.path.join('hparams', args.hparams_dir))
    hparams.device = args.device_post
    hf_model_id = hparams.model_name
    ...
    results_file = os.path.join(output_dir, f'{editing_method}_{model_name_abbrev}_{n}.csv')
    ...
    if cached_results is None:
        ...
        model_pre = AutoModelForCausalLM.from_pretrained(hf_model_id, torch_dtype='auto').to(args.device_pre)
        pre_edit_results = []
    ...
    for i in edit_indices:
        ...
        for j, pre_result in tqdm(enumerate(pre_edit_results), total=len(pre_edit_results), desc=f"Evaluating edit {i+1}/{len(edit_indices)}"):
            ...
            output_ids = model_post.generate(
                input_ids=msg_tokenized.to(model_post.device),
                max_new_tokens=task_config['max_new_tokens'],
                eos_token_id=terminators,
                do_sample=False,
                temperature=0,
            ...
            row_correct_pre = task_config['evaluation'](pre_result['predict'], label)
